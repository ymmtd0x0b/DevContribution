...
RSpec.describe Destroyer::ReviewedIssue, type: :model do
  describe '#call' do
    ...
    it 'ユーザーがレビューしている Issue 内、「他のユーザーが参照していないもの」は「削除する」こと' do
      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue') do |issue|
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) { |pr| pr.reviews.create!(user: alice) })
      end

      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue (作成者がアリス)', user: alice) do |issue|
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) { |pr| pr.reviews.create!(user: alice) })
      end

      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue + Issue にアリスがアサインしている') do |issue|
        issue.assigns.create!(user: alice)
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) { |pr| pr.reviews.create!(user: alice) })
      end

      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue + PullRequest にアリスがアサインしている') do |issue|
      ...
      expect { reviewed_issue_destroyer.call(alice) }.to change { alice.reviewed_issues.count }.from(4).to(0)
                                                     .and change { Issue.count }.from(4).to(0)
    end

    it 'ユーザーがレビューしている Issue 内、「他のユーザーが参照してるもの」は「削除しない」こと' do
      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue + Issue にボブがアサインしている') do |issue|
        issue.assigns.create!(user: bob)
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) { |pr| pr.reviews.create!(user: alice) })
      end

      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue + ボブもレビューしている') do |issue|
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) do |pr|
          pr.reviews.create!(user: alice)
      ...
      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue + PullRequest にボブがアサインしている)') do |issue|
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) do |pr|
          pr.reviews.create!(user: alice)
      ...
      FactoryBot.create(:issue, title: 'アリスがレビューしている Issue + 作成者のボブがデータベース上に存在する', user: bob) do |issue|
        issue.resolutions.create!(pull_request: FactoryBot.create(:pull_request) { |pr| pr.reviews.create!(user: alice) })
      end
